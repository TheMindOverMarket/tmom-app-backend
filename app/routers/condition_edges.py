...
router = APIRouter(prefix="/condition-edges", tags=["condition-edges"])

@router.post("/", response_model=ConditionEdge, status_code=status.HTTP_201_CREATED)
...
@router.get("/", response_model=List[ConditionEdge])
...
@router.get("/{id}", response_model=ConditionEdge)
...
@router.patch("/{id}", response_model=ConditionEdge)
